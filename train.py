...
IMAGE_DIR_NAME = 'input'    # <--- Имя папки с изображениями
MASK_DIR_NAME = 'output'   # <--- Имя папки с масками
...
REDUCE_LR_PATIENCE = 5      # Уменьшено с 10
REDUCE_LR_FACTOR = 0.5      # Увеличено с 0.1
...
try:
    # Получаем полные имена файлов из папки input, фильтруем по png
    ...
    for img_filename in all_image_filenames:
        ...
        # else:
        #     print(f"Предупреждение: Маска не найдена для {img_filename}")
...
if not all_ids_final:
    print(f"Ошибка: Не найдены совпадающие пары изображений/масок ({IMAGE_DIR_NAME}/*.png -> {MASK_DIR_NAME}/*.png).")
    print(f"Проверьте содержимое папок: {image_dir}, {mask_dir}")
    exit()

print(f"Найдено {len(all_ids_final)} пар изображений/масок.")

# Разделение на train/validation (используем полные имена файлов)
...
# Важно: Предобработка (нормализация) применяется ПОСЛЕ аугментаций в генераторе
...
# Используем f-string для динамического имени файла
